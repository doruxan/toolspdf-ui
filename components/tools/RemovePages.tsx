...
export default function RemovePages() {
  ...
  const [pagesToRemove, setPagesTorem] = useState('');
  ...
  const handleRemovePages = async () => {
    ...
    try {
      const pages = parsePageNumbers(pagesToRemove, totalPages);
      ...
      setProgress(60);
      const pdfBytes = await removePages(file, pages);
      setProgress(90);
      
      downloadPDF(pdfBytes, `${file.name.replace('.pdf', '')}_removed_pages.pdf`);
      setProgress(100);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-2">Remove PDF Pages</h1>
        <p className="text-muted-foreground">Delete specific pages from your PDF document</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              type="text"
              value={pagesToRemove}
              onChange={(e) => setPagesTorem(e.target.value)}
